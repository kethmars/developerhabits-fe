...
const SectionContent = styled.div`
  width: auto;
  max-width: ${props =>
        props.narrow ? PAGE_SIZES.desktop.widthNarrow : PAGE_SIZES.desktop.width}px;
  margin: 0 auto;
  padding: 0 40px;
...
const Section = ({ children, title, bgColor, narrow, style }) => {
    ...
    return (
        <SectionWrapper background={bgColor} className={colorClass} style={style}>
            {title && <SectionTitle>{title}</SectionTitle>}

            <SectionContent narrow={narrow}>{children}</SectionContent>
        </SectionWrapper>
    );
};
